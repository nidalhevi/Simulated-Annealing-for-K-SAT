        ...
        for t in range(mcmc_steps):
            ...
            if accept(delta_c, beta):
                ...
                if c <= best_c:
                    ...
                    if c == 0: #If the cost 0 zero then we reached the optimal. We can stop the algorithm and return 0
                        return best
                    
        # print(f"acc.rate={accepted/mcmc_steps} beta={beta} c={c} [best={best_c}]")
        acceptance_rate.append(accepted/mcmc_steps) ##

    # Plots, commented out when running 3SAT_properties.
    # acceptance_rate = np.array(acceptance_rate)
    # plt.plot(acceptance_rate)
    # plt.xlabel('Anneal Steps')
    # plt.ylabel('Acceptance Rate')
    # plt.title('Acceptance Rate over Anneal Steps')
    # plt.savefig('Acceptance Rate over Anneal Steps')
    # plt.clf()
